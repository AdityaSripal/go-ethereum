...
func (evm *EVM) SysCreate(caller common.Address, code []byte, gas uint64, endowment *uint256.Int, contractAddr common.Address) (ret []byte, leftOverGas uint64, err error) {
	var leftOverGasBudget GasBudget
	ret, _, leftOverGasBudget, err = evm.create(caller, code, NewGasBudget(gas), endowment, contractAddr, CREATE, false /* incrementNonce */)
	leftOverGas = leftOverGasBudget.RegularGas
	return
}
