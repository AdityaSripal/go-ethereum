...
func (st *stateTransition) preCheck(rules params.Rules) error {
	...
	if msg.BlobHashes != nil {
		...
		if rules.IsOsaka && len(msg.BlobHashes) > st.evm.ChainConfig().BlobScheduleConfig.GetMaxBlobsPerTransaction() {
...
func (st *stateTransition) execute() (*ExecutionResult, error) {
	...
	if st.evm.Config.NoBaseFee && msg.GasFeeCap.Sign() == 0 && msg.GasTipCap.Sign() == 0 {
		...
		if rules.IsLondon && !msg.IsFree() && st.evm.ChainConfig().Aura != nil {
