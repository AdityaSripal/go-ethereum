...
func (c *AuRa) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, body *types.Body, receipts []*types.Receipt, evm *vm.EVM, _ uint32, _ *bal.ConstructionBlockAccessList) {
...
func (c *AuRa) FinalizeAndAssemble(ctx context.Context, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt, evm *vm.EVM) (result *types.Block, err error) {
	...
	c.Finalize(chain, header, state, body, receipts, evm, 0, nil)
	finalizeSpanEnd(nil)
...
func (c *AuRa) IsServiceTransaction(sender common.Address, evm *vm.EVM) bool {
	...
	out, _, err := evm.Call(params.SystemAddress, *c.certifier, packed, vm.NewGasBudget(math.MaxUint64, 0), new(uint256.Int))
	if err != nil {
		panic(err)
	}
